...
ospfs_unlink(struct inode *dirino, struct dentry *dentry)
{
	...
	od->od_ino = 0;
	oi->oi_nlink--;
	if(!oi->oi_nlink && oi->oi_ftype != OSPFS_FTYPE_SYMLINK)
		change_size(oi, 0);
...
add_block(ospfs_inode_t *oi)
{
	...
	uint32_t *n_indir2, *n_indir;
	uint32_t indir2_offset, indir_offset;
	int i;

	if(!indir2_index(n))
	{
		if(!oi->oi_indirect2 && (allocated[0] = allocate_block()))
		{	
			oi->oi_indirect2 = allocated[0];
			n_indir2 = ospfs_block(oi->oi_indirect2);
			for(i = 0; i < OSPFS_BLKSIZE; i++)
				n_indir2[i] = 0;
		}
		else if (!oi->oi_indirect2)
			goto free_blocks;
		else
			n_indir2 = ospfs_block(oi->oi_indirect2);
		
		indir2_offset = indir_index(n);
		if(!n_indir2[indir2_offset] && (allocated[1] = allocate_block()))
			n_indir2[indir2_offset] = allocated[1];
		else if (!n_indir2[indir2_offset])
			goto free_blocks;
		
		n_indir = ospfs_block(n_indir2[indir2_offset]);
		indir_offset = direct_index(n);
		if(!(n_indir[indir_offset] = allocate_block()))
			goto free_blocks;
	}
	else if (!indir_index(n))
	{
		if(!oi->oi_indirect && (allocated[0] = allocate_block()))
		{
			oi->oi_indirect = allocated[0];
			n_indir = ospfs_block(oi->oi_indirect);
			for(i = 0; i < OSPFS_BLKSIZE; i++)
				n_indir[i] = 0;
		}
...
remove_block(ospfs_inode_t *oi)
{
	...
	if(!(indir_index(n) && indir2_index(n)))
	{
		if(!indir2_index(n))
		{
		...
		indir_offset = direct_index(n);
		free_block(n_indir [indir_offset]);
		n_indir[indir_offset] = NULL;
		
		if(!indir2_index(n) && direct_index(n) == 1)
		{
			free_block(n_indir2[indir2_offset]);
			n_indir2[indir2_offset] = NULL;
		}
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	if (*f_pos >= (size_t)oi->oi_size) //retval = -EFAULT;
		goto done;
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	if((*f_pos + count) > oi->oi_size)
		if(retval = change_size(oi, *f_pos+count))
			goto done;
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	{
		dentry = ospfs_inode_data(dir_oi, i);
		if(!dentry->od_ino)
			return dentry;
	}
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	if(!(tmp_node = ospfs_inode(src_dentry->d_inode->i_ino)))
		return -EIO;
	...
	for(i = 0; i < dst_dentry->d_name.len; i++)
		new_dir->od_name[i] = dst_dentry->d_name.name[i];
	new_dir->od_name[i] = 0;
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	if (!dentry->d_name.len || !dir || !dentry)
		return -EINVAL;
	...
	while(entry_ino < ospfs_super->os_ninodes && (!new_inode || new_inode->oi_nlink))
	...
	for(i = 0; i < dentry->d_name.len; i++)
		new_dir->od_name[i] = dentry->d_name.name[i];
	new_dir->od_name[i] = 0;
...
{		
	...
	if (!dentry || !dentry->d_name.len || !dir)
		return -EINVAL;
	...
	new_symlink = (ospfs_symlink_inode_t*) new_inode;
	...
	for(i = 0; i < strlen(symname); i++)
		new_symlink->oi_symlink[i] = symname[i];
	...
	if(qmark && colon)
	{
		for(i = strlen(symname) + 1; i < colon - qmark - 1; i++)
			new_symlink->oi_symlink[i] = qmark[i];
		new_symlink->oi_symlink[i] = 0;
	}
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	if(qmark && colon)
		nd_set_link(nd, (current->uid ? (colon + 1) : (oi->oi_symlink + strlen(oi->oi_symlink) + 1)));
	else
